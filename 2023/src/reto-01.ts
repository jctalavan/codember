...
export function decrypt (message: string) : string {
    ...
    let dicc = new Map<string, number>();

    const words = message.split(' ')

    words.forEach(word => {

        const wordFormatted = word.toLocaleLowerCase()
        let countAppareances = dicc.get(wordFormatted);
        if(!countAppareances)
            countAppareances = 0;

        dicc.set(wordFormatted, countAppareances + 1)  
    });
